    ...
    <div class="w-full max-w-3xl bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="p-6 space-y-6">
            ...
            @if(!$batchMode)
                ...
                @if($bookInfo)
                    <div
                        class="bg-emerald-50 rounded-lg p-5 border border-emerald-100 text-sm shadow-sm transition-all duration-300">
            ...
            @else
                ...
                12216
                151
                8630" {{ $isImporting ? 'disabled' : '' }}></textarea>
                                </div>
                                ...
                                <div class="flex items-center gap-3 pt-2">
                                    ...
                                    @else
                                        <button wire:click="startBatchImport" {{ $isImporting ? 'disabled' : '' }}
                                            class="px-6 py-2 bg-emerald-600 text-white font-medium rounded-md hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                                            <span wire:loading.remove wire:target="startBatchImport">🚀 بدء الاستيراد
                                                ({{ count($batchBooks) }} كتاب)</span>
                                            <span wire:loading wire:target="startBatchImport">جاري البدء...</span>
                                        </button>

                                        <button wire:click="resetBatch" {{ $isImporting ? 'disabled' : '' }}
                                            class="px-6 py-2 bg-white border border-gray-300 text-gray-700 font-medium rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                                            إعادة تعيين
                                        </button>
                                    @endif
                                    ...
                                    @if($isImporting)
                                        <div class="mr-auto">
                                            <span class="text-sm text-gray-500 italic">جاري الاستيراد...
                                                ({{ $currentBatchIndex }}/{{ count($batchBooks) }})</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                                    ...
                                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach($batchBooks as $index => $book)
                                                <tr
                                                    class="{{ $book['status'] === 'importing' ? 'bg-yellow-50' : ($book['status'] === 'done' ? 'bg-green-50' : ($book['status'] === 'error' ? 'bg-red-50' : ($book['status'] === 'skipped' ? 'bg-gray-50' : ''))) }}">
                                                    <td class="px-4 py-2 font-mono text-gray-500">{{ $book['id'] }}</td>
                                                    <td class="px-4 py-2 font-medium text-gray-900 max-w-xs truncate"
                                                        title="{{ $book['name'] }}">{{ Str::limit($book['name'], 40) }}</td>
                                                    <td class="px-4 py-2 text-gray-600 max-w-xs truncate" title="{{ $book['author'] }}">
                                                        {{ Str::limit($book['author'], 30) }}</td>
                                                    ...
                                                    <td class="px-4 py-2 text-center">
                                                        ...
                                                        @elseif($book['status'] === 'importing')
                                                            <span
                                                                class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 animate-pulse">
                                                                🔄 جاري الاستيراد
                                                            </span>
                ...
                <div class="mt-4">
                    <div class="flex items-center justify-between mb-2 px-1">
                        ...
                        <div class="flex gap-2">
                            ...
                            <span class="text-[10px] text-gray-400 font-mono uppercase tracking-wider">Scraper
                                Activity</span>
                        </div>
                    </div>
