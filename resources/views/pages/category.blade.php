...
@section('content')
    ...
    <div class="min-h-screen bg-[#fafafa] relative overflow-hidden">
        ...
        <section class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20" dir="rtl">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                ...
                <div class="w-full md:w-96">
                    <form action="{{ route('categories.index') }}" method="GET" class="relative">
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="إبحث عن قسم..."
                            class="w-full px-6 py-4 bg-white border-2 border-gray-100 rounded-2xl shadow-sm focus:border-[#2C6E4A] focus:outline-none transition-all pr-12 text-[#1a3a2a] placeholder-gray-400">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-4">
                            <svg class="h-6 w-6 text-[#2C6E4A]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if($search)
                <div class="mb-8">
                    <p class="text-gray-600">نتائج البحث عن: <span class="font-bold text-[#2C6E4A]">{{ $search }}</span></p>
                    <a href="{{ route('categories.index') }}" class="text-xs text-red-500 hover:underline">مسح البحث</a>
                </div>
            @endif
