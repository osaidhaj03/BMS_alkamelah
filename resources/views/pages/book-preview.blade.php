...
<body class="min-h-screen" style="background-color: var(--bg-body);">
    <div dir="rtl">
        ...
        <x-book.header-book :book="$book ?? null" />

        <!-- Main Layout -->
        <div class="flex" style="padding-top: 0px;">
            <!-- Sidebar Component - Pass chapters data -->
            <x-book.table-of-contents :chapters="$chapters ?? collect()" :book="$book ?? null"
                :currentPage="$currentPage ?? null" />

            <!-- Main Content Component - Livewire for smooth navigation -->
            <livewire:book-content-reader :bookId="$book->id" :pageNumber="$currentPageNum ?? 1" :book="$book" />
        </div>
    </div>
    @livewireScripts
</body>
