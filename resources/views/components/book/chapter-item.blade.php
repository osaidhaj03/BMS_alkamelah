...
@props(['chapter', 'book' => null, 'currentPage' => null, 'originalPageMap' => [], 'level' => 0])
...
<li class="toc-item" data-chapter-id="{{ $chapter->id }}">
    <div class="flex items-start gap-1">
        ...
        <a href="{{ route('book.read', ['bookId' => $book?->id, 'pageNumber' => $chapter->page_start ?? 1]) }}"
            class="flex-1 p-2 rounded-lg transition-colors block {{ $isActive ? '' : 'hover:bg-gray-50' }}"
            ...
            <div class="flex justify-between items-start gap-2">
                ...
                @if($chapter->page_start)
                    @php
                        $displayNum = $originalPageMap[$chapter->page_start] ?? $chapter->page_start;
                    @endphp
    ...
    @if($hasChildren)
        <ul class="children-list mr-4 mt-1 space-y-0.5 border-r-2 border-green-100 pr-2 hidden"
            ...
            @foreach($chapter->allChildren as $child)
                <x-book.chapter-item :chapter="$child" :book="$book" :currentPage="$currentPage"
                    :originalPageMap="$originalPageMap" :level="$level + 1" />
            @endforeach
        </ul>
    @endif
</li>