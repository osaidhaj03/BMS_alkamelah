...
<header class="book-header">
    ...
    <div class="header-main-row">
        ...
        <!-- More Actions (Three Dots) & Stats -->
        <div class="actions-container flex items-center gap-3">
            <!-- View Counter -->
            <div class="hidden lg:flex items-center gap-1 text-gray-500 bg-white border border-gray-200 px-3 py-1.5 rounded-lg" title="عدد المشاهدات">
                ...
                <span class="font-bold text-xs">{{ number_format($book->views_count ?? 0) }}</span>
            </div>

            <!-- Favorite Button -->
            @if($book)
                <livewire:components.favorite-button :model="$book" />
            @endif
        </div>
    </div>
</header>
